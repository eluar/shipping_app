...
//   targetElement: Otional. Set the DOM element wich will be 
//   targetting the elements of data.
...
(function($) {
	...
	var defaults = {
		...
		onselect: null,
                targetElement: null, //Luar added the targetElement for several colls support
		...
		show: function() {
			...
			collection.each(function(model) {
				...
				if ((v.indexOf(str) == 0 || (options.noCase && v.toLowerCase().indexOf(str_lower) == 0))
					|| (v.indexOf(str) == 0 || (options.noCase && v.toLowerCase().indexOf(str_lower) > 0)))
					result.push(model);
			});
			...
			for (var i = 0; i < result.length; i++) {
				...
				li_em.mousedown(function() {
					...
					if (options.onselect) {
						options.onselect(model, targetElement); //Luar added the targetElement for several colls support
					} else {
